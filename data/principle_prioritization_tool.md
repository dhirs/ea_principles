...
Each principle is scored on three axes. Every axis asks the same question, framed for loss aversion: **"what does it cost the org if we do NOT implement this principle?"** Asking "what's the value of implementing it?" instead invites optimism and double-counting. Asking what breaks if you skip it produces sharper, more honest scores.

The three axes are **not blended into a weighted average.** They are a strict hierarchy. Higher rungs always outrank lower rungs, and no amount of a lower axis can compensate for a higher one:
...
Each axis is a distinct harm-bearer if the principle is absent: the **regulator** (Legal), the **customer** (CX), and the **business's own money** (Cost).

To order two principles, compare them on **Legal first**. The higher Legal score wins outright — nothing below it can pull the lower one ahead. Only when Legal *ties* do you look at Customer Experience. Cost is the final tiebreaker and **never promotes a principle above a higher rung** — it only orders principles that are already tied on Legal and CX.

Formally: rank by the tuple `(Legal, CX, Cost)` in descending order, compared left to right; the first axis on which they disagree decides.
...
> **What this ladder deliberately leaves out.** It measures only the **cost of absence** (what you lose by NOT having the principle). It does NOT measure the **cost to build/adopt** the principle — development effort, the time local teams spend learning central utilities, the change to how developers work. That is real, but it is a *feasibility and sequencing* input, not a priority-of-absence one, and it is captured by the catalogue's own metadata (`ownership.tier`, `maturity_level`) — not on this ladder. Adoption effort is also near-universal across EA principles, so it does not discriminate between them. Keep build cost out of the priority scoring; bring it in only when you sequence the already-ranked list against team capacity.

---

## The three axes

Score each principle 1–5 on every axis using the anchors verbatim — do not invent new shades of meaning. The anchors are ordered to match the ladder (Legal, CX, Cost).
...
1. Score each principle 1–5 on all three axes, evidenced per the rule above.
2. Rank by the tuple `(Legal, CX, Cost)` descending, compared left to right. The first axis on which two principles disagree decides their order. This is the strict priority order.
...
The order is universal — the ladder (Legal > CX > Cost) does not get re-weighted per org. What differs per org is the *evidenced scores*, not the ranking of the axes.
...
**GO3B2-02 (govern read access + retention on observability traces).** Intrinsic `impact_level` is High, but that is not its priority — its priority comes from the axes. The traces carry PII, so GDPR/CCPA access-and-retention obligations apply directly → **Legal 4** (sourced: the PII inventory shows ticket data flows into traces; counsel confirms retention scope). CX 1 (internal-facing), Cost 2. Tuple `(4,1,2)`. No other principle scores Legal 4, so it ranks **first** — even though its CX is the lowest possible and it is only a `scaling`-tier principle.

GO3B2-02 hard-depends on **GO3B2-01** (the observability SDK that emits the traces). GO3B2-01's own tuple is `(3,3,3)` — it would rank lower on its own. But it is the hard prerequisite of the top principle, so it **inherits rank 1** and builds immediately before GO3B2-02.

Contrast: **GC3B3-01 (prompt caching).** Suppose the org is bleeding on inference. The temptation is to call this critical. But Cost is the bottom rung. Even a sourced Cost 5 (finance quotes \$1.2M/yr of cacheable waste) cannot lift it past anything with Legal or CX weight. Its tuple is `(1,2,4)` — Legal 1, CX 2 — so it lands at the **bottom** of the build order, alongside the other pure-cost principles. The cost bleed is real, but under this ladder cost never jumps the queue. That is the rule working as intended, not against you.

Same three scores, one strict order, no weighting knob to argue over.
...
| Rank | Principle ID | Title | Legal | CX | Cost | Inherited? | Notes |
|---|---|---|---|---|---|---|---|
| 0 | GO3B2-01 | Centralised Observability SDK | 3 | 3 | 3 | yes — hard prereq of GO3B2-02 | Build first to unblock rank-1 legal item. |
| 1 | GO3B2-02 | Govern trace access + retention | 4 | 1 | 2 | no | Legal-4: PII in traces. Top priority. |
| … | … | … | … | … | … | … | … |
...
The three axes are not equally stable over time:
...
- **Cost** changes as the workload portfolio grows — but a rising Cost score only re-orders principles already tied on the two rungs above it.

Recommended cadence: full re-assessment quarterly during the first year of catalogue adoption, annually thereafter. Always re-assess immediately after a regulatory change or a strategic roadmap commitment.
...
prioritization_catalog:
  ...
  ladder: ["legal", "customer_experience", "cost"]  # fixed order, not re-weighted
  thresholds:
  ...
  principles:
    - rank: 0
      ...
      scores: { legal: 3, customer_experience: 3, cost: 3 }
    ...
    - rank: 1
      ...
      scores: { legal: 4, customer_experience: 1, cost: 2 }
      inherited_from: null
...
- **It does not score cost-to-build or adoption effort.** Development effort, the time teams spend learning central utilities, and the change to how developers work are *feasibility/sequencing* inputs, captured by `ownership.tier` and `maturity_level` — not priority-of-absence inputs. They are also near-universal across EA principles, so they do not discriminate. Keep them off the ladder; apply them only when sequencing the ranked list against team capacity.
...
- **It does not let you re-weight the ladder.** The axis order (Legal > CX > Cost) is fixed. Per-org judgment lives in the *evidenced scores*, not in the ranking of the axes.
