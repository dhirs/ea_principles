...
## GENREL — Reliability [FULLY WALKED 2026-06-27 — GENREL01–06 all closed; 2 standards (GR3B1-01, GR3B2-01)]
...
> Note: GENREL04 (Prompt management) walked 2026-06-23 — both BPs not_promoted (BP01 prompt catalog → GO3B1-01; BP02 model catalog → GO1B1-06). GENREL05 (Distributed availability) walked 2026-06-27 — all three BPs not_promoted (BP01/BP02 base-WAF REL10; BP03 agent region-completeness → ADR). GENREL06 (Distributed compute tasks) walked 2026-06-27 — its one BP not_promoted (model-customization fault tolerance; out of serving paradigm + base-WAF REL10/REL11). **Reliability pillar now FULLY WALKED.**
...
### GENREL05 — Distributed availability [VERIFIED — walked 2026-06-27; all three BPs not_promoted]

> *"GENREL05: How do you create systems for high availability across multiple regions and availability zones to mitigate the impact of localized failures or outages?"* (live pages fetched 2026-06-27 — genrel05-bp01.html, genrel05-bp02.html, genrel05-bp03.html. mapping_state **verified**.)
>
> Three BPs, all AWS risk **Medium**. Multi-region DR for inference, embedding data, and agent infrastructure.

| Best Practice | Title | Our standard |
|---|---|---|
| GENREL05-BP01 [VERIFIED — walked 2026-06-27] | Load-balance inference requests across all regions of availability | **not_promoted 2026-06-27 (user-directed).** GenAI-Lens relabel of base-WAF multi-region/multi-AZ inference serving: Bedrock cross-Region inference profiles or SageMaker multi-AZ endpoints + auto-scaling + health checks/failover + cross-Region monitoring. Generic horizontal-scaling DR ("inference request" = any request); **vendor menu**; **no commitable repo artefact** (routing/topology is landing-zone). BP's own Resources cross-reference base-WAF **REL10-BP01** + **REL04-BP01** — absorbed. step_promotion **3/3/3/3 → not_promote**. No ADR. See data/authored/GENREL05-BP01.md. |
| GENREL05-BP02 [VERIFIED — walked 2026-06-27] | Replicate embedding data across all regions of availability | **not_promoted 2026-06-27 (user-directed).** Cross-Region replication of vector/knowledge-base data: S3 CRR + OpenSearch cross-cluster replication + Glue sync, with data-sovereignty constraints + S3 Intelligent-Tiering. Generic cross-Region data replication ("embedding store" = any datastore); **vendor menu**; **no repo artefact**; absorbed by base-WAF **REL10** (cross-refs REL04/07/10). Adjacent angles land cross-pillar: data residency → GDPR/compliance + GS1B3-02 ingest gate; replication cost → GENCOST. step_promotion **3/3/3/3 → not_promote**. No ADR. See data/authored/GENREL05-BP02.md. |
| GENREL05-BP03 [VERIFIED — walked 2026-06-27] | Verify that agent capabilities are available across all regions of availability | **not_promoted 2026-06-27 (user-directed).** **The one BP of GENREL05 with a genuinely agent-distinct failure mode:** regional failover that is transparent for a stateless service can **silently degrade an agent** — if its tool surface (APIs/functions) or model access isn't region-complete in the failover Region, the agent runs with a missing capability instead of erroring; the failover unit is the agent's whole tool + model surface, which base-WAF REL10 doesn't address. Still not_promoted: AWS mechanism (deploy APIs in N Regions + CloudFront/Route 53 latency routing) is generic infra; the agent-distinct part is absorbable as a **GO2B3-01 tool-binding config extension** (a `target_regions` completeness check), not a new standard; model-access-per-Region rides **GO1B1-06**. Gets an ADR (cf. GENREL01) — recommendation: treat the tool+model surface as the failover unit, make region-completeness checkable, fail closed not silent. step_promotion **3/3/3/3 → not_promote**. Recommendation at **data/adr/GENREL05-agent-region-completeness.md**. See data/authored/GENREL05-BP03.md. |

**GENREL05 status:** BP01 → not_promoted (base-WAF REL10); BP02 → not_promoted (base-WAF REL10); BP03 → not_promoted (GO2B3-01 extension + ADR). **Focus area CLOSED — zero new standards. Pattern: distributed availability is base-WAF multi-region DR wearing GenAI nouns (inference / embedding / agent), with one agent-distinct sliver (silent capability loss on failover) captured as an ADR rather than a standard.**

### GENREL06 — Distributed compute tasks [VERIFIED — walked 2026-06-27; one BP not_promoted]

> *"GENREL06: How do you design high-performance distributed computation tasks to maximize successful completion?"* (live pages fetched 2026-06-27 — genrel06.html, genrel06-bp01.html. mapping_state **verified**.)
>
> Ships **one** BP. Entirely about model customization (pre-training / fine-tuning / distillation) fault tolerance.

| Best Practice | Title | Our standard |
|---|---|---|
| GENREL06-BP01 [VERIFIED — walked 2026-06-27] | Design for fault-tolerance for high-performance distributed computation tasks | **not_promoted 2026-06-27 (user-directed).** AWS risk **High**. Entirely **model customization** — long-running (days–months) distributed training jobs (pre-training / continuous pre-training / fine-tuning / distillation) made fault-tolerant via checkpointing to S3/FSx, auto-resume from last checkpoint, GPU/node health monitoring + auto node replacement, on SageMaker HyperPod (EKS/Slurm) / Bedrock managed customization / AWS Batch. **Cleanest not_promote of the pillar:** (1) **out of the dominant serving paradigm** — only bites teams that self-host model training; the API-consuming majority has nothing to gate; (2) generic distributed-HPC fault tolerance ("model customization job" = any HPC job) — BP cross-references base-WAF **REL10-BP02 / REL11-BP01 / REL11-BP03**; (3) **vendor menu** (HyperPod / Bedrock / Batch / FSx / EKS / Slurm); (4) **no workload-repo artefact** — checkpointing lives in training scripts + cluster config, infra-level. Consistent with GENOPS04 (FMOps training vendor menu) + GENSUS01-BP02 (customization cost out of walk). Training infra is out of catalogue remit. step_promotion **3/3/3/3 → not_promote**. No ADR. See data/authored/GENREL06-BP01.md. |

**GENREL06 status:** BP01 → not_promoted (out of serving paradigm + base-WAF REL10/REL11). **Focus area CLOSED — zero new standards.**

---

**GENREL pillar FULLY WALKED 2026-06-27 — 2 standards (GR3B1-01, GR3B2-01).** Six focus areas: GENREL01 (Throughput quotas) → not_promoted/ADR; GENREL02 (Network reliability) → not_promoted/base-WAF REL02; GENREL03 (Prompt remediation & recovery) → **GR3B1-01 + GR3B2-01** (the only promotes); GENREL04 (Prompt management) → both not_promoted/ADRs (GO3B1-01, GO1B1-06 twins); GENREL05 (Distributed availability) → all 3 not_promoted (base-WAF REL10; BP03 agent region-completeness ADR); GENREL06 (Distributed compute) → not_promoted (model-customization, out of remit). The Reliability pillar yields exactly one enforceable GenAI-distinct slice — **runtime non-determinism recovery** (GENREL03's graceful-recovery + agentic-timeout pair). Everything else is base-WAF redundancy/DR wearing GenAI nouns or model-training infra outside the serving paradigm. Three AWS pillars now fully walked with explicit stamps: GENOPS, GENSEC, GENPERF, plus GENREL.
...
**GENCOST pillar FULLY WALKED 2026-06-27 — 7 standards (GC1B1-01, GC2B2-01, GC3B1-01, GC3B2-01, GC3B3-01, GC4B1-01, GC5B1-01).** Full BP enumeration verified against the live cost-optimization.html + gencostNN.html pages 2026-06-27: exactly **5 focus areas** (GENCOST01–05, no GENCOST06) and **9 BPs** total — GENCOST01 (1 BP), GENCOST02 (2), GENCOST03 (4), GENCOST04 (1), GENCOST05 (1). Every BP accounted for; 7 promoted, 2 not_promoted (GENCOST02-BP01 absorbed by GC1B1-01; GENCOST03-BP04 vendor-tag micro-optimisation). The last `unverified` anchor (GENCOST03-BP02 → GC3B2-01) was confirmed against the live page 2026-06-27 and flipped to `verified` (GC3B2-01 → v1.0.2). See decisions.md (latest).
