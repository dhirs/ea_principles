...
**Data:** Supabase project `thnxknvcahqktpbpqvbg` — `apollo_company_raw` (superset, 3,816) → `apollo_company_universe` (**2,983 live targets**) → `apollo_company_scores` (Stage 4/5 scores, empty) + static `apollo_naics` reference.
...
- **Target set — `stage4_target_technologies.json`.** 33 CDP names (this ICP wants *any* CDP) + 466 MAP, each validated verbatim against the CSV. The 33 CDP names were curated by me from the CSV and **accepted by Dheeraj as-is** (incl. borderline: Hightouch, Snowplow, Optimove, Netcore Cloud, Cooladata, Sitecore Engagement Cloud, Adobe Experience Platform).
...
- **Open threads:** (1) 18 long-tail MAP uids unresolved; (2) **fit-model tension** — we discussed collapsing the 5 fit areas to 2 (technology match + an agentic need-inference for greenfield "no CDP yet" accounts), but `adr/2026-07-17-stage4-fit-areas.md` still describes 5 areas (Accepted); unreconciled. (3) Technology discovery finds **displacement** targets (already run a CDP/MAP), *not* greenfield — correct for this ICP's "any CDP" ask, but the means/capability/greenfield paths (old areas 2–5) are not built.
...
**Next:** Stage 4 Area 1 is **done for CDP**. Open options, not yet started: (a) score the **MAP** set the same way (466 names / uids in `stage4_target_technologies.json`, not yet probed); (b) build **areas 2–5** (need/means/capability/trigger per `adr/2026-07-17-stage4-fit-areas.md`) — these need enrichment + non-Apollo sources, and the 5-areas-vs-2 tension noted above is still unreconciled; (c) Stage 5 intent. Reminder: `apollo_company_scores.score` is constrained 0–100 — Area 1 counts (1–5) fit, but a future combined score must stay on that scale or the check changes deliberately.
- **Open cost question — RESOLVED 2026-07-17: filter-first.** Verified 2026-07-17: the company-search payload (31 fields, all stored verbatim) carries **no technographics, no funding, no department headcount** — search is a thin discovery endpoint charging 1 credit per *page of 100*, while `organizations_enrich` returns the full record at ~1 credit *each*. So enriching all ~2,983 would cost ~2,983 credits vs ~2,500 available. **Decision: do not enrich the whole universe — discover the CDP/MAP subset by technology and enrich only those, caching payloads so re-runs cost 0** (see the Stage 4 state entry below and `stage4_fit.md` → *Execution*).
